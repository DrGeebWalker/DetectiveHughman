...
<html>
	<head>
		...
		<script>
			...
			function hasArtThree(){
				const helditems = "<available_items/>".split(" ");
				if(!helditems.includes("ARTTHREE")){
	...
	<body onload="hasArtThree()">
		<h1 style="text-align:center"> Detective Hughman and the Art Gallary Murder </h1>
		
		<img id = "img" src="locations/sealpainting.jpg" class="location"/>
		
		<div class="plaqueThree" id="plaq" style="display:none" onclick="artInv()"></div>
		
		<div class="added-item" id="takeArt" style="display:none">
			<p>My Ex Husband Drowning in Hell by Hydra Bass</p>
			<br>
			<br>
			<p>Detective Hughman: O_O</p>
			<br>
			<br>
			<p> My Ex Husband Drowning in Hell Added to Inventory</p>
			<br>
			<br>
			<button onclick="addArtThree()">Continue</button>
		</div>
